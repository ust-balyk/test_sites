...
try {
    ...
    if (!empty($_FILES['image']['name'])) {
        ...
        if (!move_uploaded_file($file['tmp_name'], $target)) throw new RuntimeException('Не удалось сохранить файл');
    ...
    if ($outer_id === 'new') {
        $new_outer = 'ART-' . time();
        $sql = "INSERT INTO products (outer_id, title, price, description, image, created_at) VALUES (?, ?, ?, ?, ?, NOW())";
        ...
        if ($imagePublicPath) {
            $sql = "UPDATE " . TABLE_NAME . " SET title = ?, price = ?, description = ?, image = ? WHERE outer_id = ?";
            $stmt = $pdo->prepare($sql);
