...
class Image
{
    ...
    private function floodRemoveBackground(GdImage $img, int $w, int $h, array $target, int $thresholdSq): void
    {
        ...
        // Используем альфу как маркер посещённости:
        // после imagesetpixel() пиксель становится прозрачно-фиксированным,
        // поэтому повторно он уже не будет попадать в обработку.
        $stack = [[0,0], [$w-1,0], [0,$h-1], [$w-1,$h-1]];
        ...
        while (!empty($stack)) {
            ...
            // Если уже "помечено" как прозрачное — не трогаем
            if ($alpha >= 127) continue;
