...
class PageController
{
    ...
    static function product()
    {
        ...
        // Берем все товары из той же категории
        $category_slug = $product['slug'];
        $related = $cache['by_category'][$category_slug] ?? [];

        // Исключаем текущий товар из списка похожих, чтобы не предлагать купить то же самое
        $related = array_filter($related, function($item) use ($id) {
            return $item['id'] !== $id;
        });

        shuffle($related);
        $related = array_slice($related, 0, 4); // Берем, например, только 4 штуки
